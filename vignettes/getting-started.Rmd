...
# Introduction <img src="../man/figures/logo.svg" align="right" height="139" alt="metaMultiverse hex logo" />
...
**Want more depth?** See `vignette("multiverse-theory-practice")` for comprehensive theoretical background and advanced usage.
...
- **N (Non-equivalent)**: Different research questions (creates separate multiverses)

For detailed theoretical background on the E/U/N framework, see `vignette("multiverse-theory-practice")`.
...
## N-Type Custom Factors: Separate Multiverses

Custom factors can also use Type N decisions to create separate multiverses (new in v0.2.3):

```{r define-custom-n, eval=FALSE}
factor_setup_n <- data_validated %>%
  define_factors(
    Population = "wf_3|U",

    # N-type custom factor creates separate multiverses
    RiskOfBias = list(
      column = "wf_7",
      decision = "N",
      groups = list(
        low_only = "low risk",
        low_moderate = c("low risk", "some concerns"),
...
This creates **three separate multiverses** (one per risk level), not one combined multiverse. Each is analyzed independently because they represent different research questions about study quality.
...
Methods include:
- **Standard**: FE, REML, Paule-Mandel, Hartung-Knapp/Sidik-Jonkman
- **Bias correction**: PET-PEESE, p-uniform*, UWLS, WAAP
- **Dependency modeling**: Three-level, RVE
- **Bayesian**: bayesmeta (optional, slow)
- **On demand**: `register_metafor_estimators()` adds DerSimonian-Laird (`dl`) and the Hartung-Knapp-Sidik-Jonkman variants `dl_hksj`, `reml_hksj`, `pm_hksj`
...
## Pre/Post Data: The Effect-Size Metric as a Factor

The pipeline above starts from precomputed `yi`/`vi`. When trials report arm-level pre/post means and SDs, the effect-size metric is itself an analytical decision: post-test SMD, SMD of change scores (with an assumed pre-post correlation for the change SD), standardized mean change, or a reported adjusted difference. `run_pre_post_multiverse()` runs the standard pipeline once per metric and stacks the results, so the metric enters the multiverse as a "how" factor next to `ma_method` and `dependency`.
...
# Effect sizes for one metric
head(compute_pre_post_es(d, "change_smd_r0.5")[, c("study", "yi", "vi")], 3)

# The metric as a factor: one run per row of es_grid, results stacked
...
mv$results %>%
  group_by(es_metric) %>%
  summarise(n_analyses = n(), median_g = round(median(b), 3), .groups = "drop")

# The metric and imputed-post-SD rule appear as how factors in the plot
plot_spec_curve(mv, interactive = FALSE)
```

Columns 1 and 2 are the two arms; positive means arm 1 scored higher at post-test (or improved less). `k_studies` counts distinct studies per analysis, `studies_in_set` names them. Studies without a reported post SD are flagged with `post_sd_imputed = TRUE`, and `imputed_post_sd` (`"borrow"`, `"exclude"`, `"change_smd"`) decides how they enter the post-test metrics. See `?compute_pre_post_es` for all metrics and columns.
...
See `vignette("multiverse-theory-practice")` for comprehensive coverage of the E/U/N framework, advanced interpretation, and reporting guidelines.

**For hands-on tutorial with real data:**
See the [blog post tutorial](https://constantinyvesplessen.com/posts/metamultiverse-tutorial.html) for a step-by-step walkthrough using Metapsy API data.
