...
**Want a quick start?** See `vignette("getting-started")` for a streamlined introduction to the package.
...
-   Effect size metric (when multiple options available; for arm-level pre/post data see `run_pre_post_multiverse()`, which varies post-test SMD, change-score SMD, standardized mean change and adjusted differences as a factor)
...
### N-Type Custom Factors: Separate Multiverses (v0.2.3+)

Custom factors can also be designated as Type N to create separate multiverses:

```{r custom-n-type, eval=FALSE}
factor_setup_n <- data_validated %>%
  define_factors(
    Population = "wf_3|U",

    # N-type custom factor: Each group becomes a separate multiverse
    RiskOfBias = list(
      column = "wf_7",
      decision = "N",  # Non-equivalent: different research questions
      groups = list(
        low_only = "low risk",
...
This creates **three separate multiverses**:
1. Low-risk studies only
2. Low + moderate risk studies
3. All studies regardless of risk

Each multiverse is analyzed independently because they represent fundamentally different research questions about study quality. Type N prevents the "total_" aggregation option from being added, ensuring clean separation between research questions.

**When to use N-type custom factors:**

-   Different quality standards represent different research questions
-   Distinct population groups (e.g., clinical vs. subclinical)
-   Conceptually different intervention types that shouldn't be pooled
...
DerSimonian-Laird and the Hartung-Knapp-Sidik-Jonkman variants of DL, REML and Paule-Mandel are available on demand: `register_metafor_estimators()` adds `dl`, `dl_hksj`, `reml_hksj` and `pm_hksj` to this list for the current session.
...
**For quick start:**
See `vignette("getting-started")` for a streamlined introduction to the package with essential workflow steps.

**For hands-on tutorial with real data:**
See the [blog post tutorial](https://constantinyvesplessen.com/posts/metamultiverse-tutorial.html) for a comprehensive walkthrough using Metapsy API data.
