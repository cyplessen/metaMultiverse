...
add_pet_peese_corrected <- function(data) {
  ...
  pet_peese_corrected <- data %>%
    ...
    mutate(b = ifelse(b < 0, 0, b),  # Correct negative effect size estimates
...
```{r}
# Generate plot
plot_descriptive_spec_curve(res, 
                            ylim_upper = 2, 
                            ylim_lower = -2, colorblind_friendly = FALSE)
```
# plotly version
...
### `plot_descriptive_spec_curve_plotly`
```{r}
...
```{r}
# Generate plot
plotly_descriptive_spec_curve(res, ylim_upper = 2, ylim_lower = -2, colorblind_friendly = FALSE)
```
...
compute_density_colors <- function(data, x, y, colorblind_friendly = TRUE) {
  dens_values <- densCols(
    x = data[[x]],
    y = data[[y]],
    colramp = if (colorblind_friendly) colorRampPalette(viridis::viridis(256)) else colorRampPalette(rev(rainbow(10, end = 4/6)))
  )
  data$density <- dens_values
  return(data)
}
