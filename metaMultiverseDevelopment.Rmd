...
plot_descriptive_spec_curve <- function(data, 
  ...
  # fill_levels <- cut(data$k, breaks = fill_quantiles, include.lowest = TRUE, labels = 1:num_levels)
    ...
    # Step 1: Assign quantile levels using the helper function
  data <- assign_quantile_levels(data, k_column = "k", num_levels = 9)
  
  # Step 2: Choose color palette based on argument
  ...
  # scale_x_continuous(position = "bottom") +
  # scale_y_discrete(labels = levels(yvar)) +
  # scale_fill_viridis_d(option = "D", name = "Number of Studies", na.value = "white") +
...
# Step 1: Dynamically create quantile breaks
assign_quantile_levels <- function(data, k_column, num_levels = 9) {
  # Ensure k is numeric
  if (!is.numeric(data[[k_column]])) stop("`k_column` must be numeric.")
...
# Example usage
data <- assign_quantile_levels(data, "k", num_levels = 9)

# Step 3: Integrate Viridis Colors
viridis_palette <- viridis(n = length(unique(data$fill_levels)), option = "C")  # Adjust option for aesthetics

# Step 4: Plot
tile_plot <- ggplot(data, aes(x = xvar, y = as.factor(yvar), fill = fill_levels)) +
  geom_raster() +
  scale_fill_manual(
...
plot_VoE <- function(
  ...
  # Add density coloring using Viridis
  dens_values <- densCols(
    x = data[[x]],
    y = data[[y]],
    colramp = colorRampPalette(viridisLite::viridis(256))
  )
